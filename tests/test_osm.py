...
import sys
sys.path.append('..')
from app.tools.osm_tool import get_osm_data, run_osm_data_tool  # replace with your actual module name
...
def test_empty_result(mock_query, valid_area_response):
    mock_query.side_effect = [valid_area_response] + [MagicMock(nodes=[], ways=[], relations=[])]
    res = get_osm_data("Paris", ["unused_feature"])
    assert res["status"] == "empty"
    assert "No data found" in res["message"]

@patch("your_module.run_overpass_query")
def test_success_result(mock_query, valid_area_response, valid_feature_response, tmp_path, monkeypatch):
    # Prepare output dir
