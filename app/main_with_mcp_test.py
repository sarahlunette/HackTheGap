...
def reasoning_fn(state):
    ...
    try:
        ...
        if not isinstance(reasoning_json, dict):
            logger.error(f"Reasoning JSON is not a dict: {reasoning_json}")
            reasoning_json = None
    except Exception as e:
        logger.error(f"Reasoning JSON parse error: {e}")
        reasoning_json = None
    print(reasoning_json)
    # Ensure consistent keys (insert your snippet here)
    # entities = reasoning_json["entities"]
    # print(entities)
    # if entities is None or not isinstance(entities, dict):
    #     entities = {}

    # # preserve everything, just ensure the keys you need exist
    # for k in ["lat", "lon", "date", "radius"]:
...
def tool_earth_engine_fn(state):
    ...
    if reasoning is None:
        logger.error("tool_earth_engine_fn: reasoning is None")
        return {"tool_result": "Tool not called — reasoning missing."}
    entities = reasoning.get("entities")
    if entities is None:
        logger.error("tool_earth_engine_fn: entities is None")
        return {"tool_result": "Tool not called — entities missing."}
    ...
    if lat is not None and lon is not None and date is not None:
        ...
        logger.warning(
            f"tool_earth_engine_fn: insufficient parameters lat={lat}, lon={lon}, date={date}"
        )
...
def run_test(user_msg: str):
    outputs = graph_app_.invoke({"user_msg": user_msg})
    return outputs
...
if __name__ == "__main__":
    state = {"user_msg": "Cyclone irma, saint-martin"}
    reasoning = reasoning_fn(state)
    print(reasoning)
    state = {"user_msg": "Cyclone irma, saint-martin", "reasoning": reasoning}
